...
The preprocessing/feature engineering for these models can be observed in the `02_recipes.R` script. 4 different recipes were created:

-   `recipe1_kitcken_sink`: This is my basic recipe, with appropriate variables removed (the original variable of likes, the id of a comment, the id of a parent comment, the commenter's username, the comment itself, and any variables with zero variance), numerical predictors transformed with a Yeo Johnson transformation, factor variables dummy coded, and predictors centered/normalized.

-   `recipe2_kitchen_sink_trees`: This is the trees version of my basic recipe, with appropriate variables removed (the original variable of likes, the id of a comment, the id of a parent comment, the commenter's username, the comment itself, and any variables with zero variance), factor variables one-hot encoded, and predictors centered/normalized.

```{=html}
<!-- -->
```
-   `recipe3_transformed_interactions`: This is my complex recipe, with numerical predictors transformed with a Yeo Johnson transformation, factor variables dummy coded, predictors centered/normalized, and interactions included.

-   `recipe4_transformed_trees`: This is the trees version of my complex recipe, only utilizing the variables included in recipe 3. Numerical predictors transformed were with a Yeo Johnson transformation, factor variables were dummy coded, and predictors were centered/normalized.
