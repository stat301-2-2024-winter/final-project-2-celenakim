...
-   Boosted tree (`xgboost` engine)
    ...
    -   Minimum number of data points in a node for splitting was explored over $[1,30]$ with 4 levels
...
The null, linear, elastic net, and k nearest neighbors models used the preprocessing stored in `recipe1_kitcken_sink` and `recipe3_transformed_interactions`. We take care to remove appropriate variables (`rings` and variables with zero variance), dummy code any factor variables, add complexity by including some interactions, and standardize all numerical variables.
