...
As shown in @fig-rest-raw-inspection, all the numerical predictor variables such as the presence of anger within comments are heavily skewed right. Log transformations will not work as these variables contain values of 0, and a square root transformation does not seem to help to normalize their distributions. Thus, these variables will undergo a BoxCox transformation within a recipe step.
...
-   **recipe 4: transformed & interactions (trees)**: This is the trees version of my complex recipe, only utilizing the variables included in recipe 3. Numerical predictors transformed were with a BoxCox transformation, factor variables were dummy coded, and predictors were centered/normalized.

The null, logistic regression, elastic net, and k nearest neighbors models used recipes 1 and 3.

The k-nearest neighbors, random forest, and boosted tree models used recipes 3 and 4.

## Model Building & Selection

## Assessment Metric
I decided accuracy would be the metric by which I would compare models.
...
tbl_result_accuracy
```
As can be seen in @fig-model-results, here are the best performing model results in terms of accuracy. 

## Tuning Parameters
Here are the visualizations of the tuning of my elastic net, k nearest neighbors, random forest, and boosted tree models, as well as the selected best values.
...
```{r}
#| label: fig-selectbest-tuned-en-a
#| fig-cap: The best tuning parameter values for the elastic net model a

select_best(tuned_en_a, metric = "accuracy") |> 
  knitr::kable()
```
As seen in @fig-autoplot-tuned-en-a and @fig-selectbest-tuned-en-a, the best parameters to obtain the highest accuracy for the elastic net model a is a penalty of 0 and mixture of 0.7625.
...
```{r}
#| label: fig-selectbest-tuned-en-b
#| fig-cap: The best tuning parameter values for the elastic net model b

select_best(tuned_en_b, metric = "accuracy") |> 
  knitr::kable()
```
@fig-autoplot-tuned-en-b and @fig-selectbest-tuned-en-b show that the best parameters to obtain the highest accuracy for the elastic net model b are also a penalty of 0 and mixture of 0.7625.
...
```{r}
#| label: fig-selectbest-tuned-knn-a
#| fig-cap: The best tuning parameter values for the k nearest neighbors model a

select_best(tuned_knn_a, metric = "accuracy") |> 
  knitr::kable()
```
@fig-autoplot-tuned-knn-a and @fig-selectbest-tuned-knn-a show that the best parameters to obtain the highest accuracy for the k nearest neighbors model a is 1 neighbor.
...
```{r}
#| label: fig-selectbest-tuned-knn-b
#| fig-cap: The best tuning parameter values for the k nearest neighbors model b

select_best(tuned_knn_b, metric = "accuracy") |> 
  knitr::kable()
```
As seen in @fig-autoplot-tuned-knn-b and @fig-selectbest-tuned-knn-b, the best parameters to obtain the highest accuracy for the k nearest neighbors model b is 34 neighbors.
...
```{r}
#| label: fig-selectbest-tuned-rf-a
#| fig-cap: The best tuning parameter values for the random forest model a

select_best(tuned_rf_a, metric = "accuracy") |> 
  knitr::kable()
```
@fig-autoplot-tuned-rf-a and @fig-selectbest-tuned-rf-a show that the best parameters to obtain the highest accuracy for the random forest model a is 15 variables randomly sampled at each split and a minimum of 2 samples required to be present in a node for further splitting to be considered.
...
```{r}
#| label: fig-selectbest-tuned-rf-b
#| fig-cap: The best tuning parameter values for the random forest model b

select_best(tuned_rf_b, metric = "accuracy") |> 
  knitr::kable()
```
@fig-autoplot-tuned-rf-b and @fig-selectbest-tuned-rf-b show that the best parameters to obtain the highest accuracy for the random forest model b is 8 variables randomly sampled at each split and a minimum of 2 samples required to be present in a node for further splitting to be considered.
...
```{r}
#| label: fig-selectbest-tuned-bt-a
#| fig-cap: The best tuning parameter values for the boosted tree model a

select_best(tuned_bt_a, metric = "accuracy") |> 
  knitr::kable()
```
As seen in @fig-autoplot-tuned-bt-a and @fig-selectbest-tuned-bt-a, the best parameters to obtain the highest accuracy for the boosted tree model a is 1 variable randomly sampled at each split, a minimum of 2 samples required to be present in a node for further splitting to be considered and a learning rate of 0.631.
...
#| fig-cap: A table of the correlations between the target variable of comment length with the numerical predictor variables.
...
After assessing my final model's performance, it was very successful, and I do think that the effort of building a predictive model paid off. Compared to the baseline model before choosing the final model, the accuracy of the random forest model a was approximately 85.09% higher than the null, indicating a notable improvement in model performance. The recipes used for these models were both the basic kitchen sink recipes, so this model improvement could not have been the result of an improved recipe. Rather, it could be attributed to the fact that the random forest model itself leverages more advanced techniques than the null to carry out these predictions.
...
correlation_tbl
```
As seen in @fig-target-var-cors, the predictors of the word count of a comment and the average words per sentence of a comment have the strongest correlations with the target variable of comment length, as those are both factors that directly impact/have a direct relationship with how long a comment would be. Thus, it was necessary to remove those predictors from my recipes.
