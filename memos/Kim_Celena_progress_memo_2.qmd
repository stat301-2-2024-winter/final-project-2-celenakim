...
Outcome variable transformation: I have performed a Yeo-Johnson transformation on my outcome variable, `likes`, as the distribution is heavily skewed right due to extreme outliers of like counts. This transformation from `likes` to `likes_yj` was carried out in its own recipe, which I then binded to the allies_train data.
...
-   Recipe 3- Recipe with Yeo-Johnson transformations and interaction terms (allies_recipe2)

-   Recipe 4- Recipe with Yeo-Johnson transformations and one hot (

## Two Model Types

1.  lm model with recipe3_interactions
...
```
For my linear model, I obtained a rmse of 0.482, and for the null model, I obtwined a rmse of 0.511. Note that these are on the scale of the Yeo-Johnson transformation.
